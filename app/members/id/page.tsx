...
const GRADES = [
  'Bianca','Mezza gialla','Gialla','Mezza verde','Verde',
  'Mezza blu','Blu','Mezza rossa','Rossa','Mezza nera','Nera'
];
...
export default function Page() {
  ...
  if (!m || m.error) {
    return <div className="card">Atleta non trovato. <Link href="/members">← Torna</Link></div>;
  }
  ...
  const save = async () => {
    const patch = {
      ...
      telefono: m.telefono,
      email: m.email,
      genitoreNome: m.genitore_nome || m.genitoreNome,
      genitoreTelefono: m.genitore_telefono || m.genitoreTelefono,
      grado: m.grado,
      fitaNumero: m.fita_numero || m.fitaNumero,
      wtGalNumero: m.wt_gal_numero || m.wtGalNumero,
      wtGalScadenza: m.wt_gal_scadenza || m.wtGalScadenza,
    };
  ...
  return (
    <>
      <div className="card" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
        ...
        <label>Sede
          <select value={m.sedeNome || 'Rozzano'} onChange={e => setM({ ...m, sedeNome: e.target.value })}>
        ...
        <label>Grado
          <select value={m.grado || 'Bianca'} onChange={e => setM({ ...m, grado: e.target.value })}>
            {GRADES.map(g => <option key={g}>{g}</option>)}
          </select>
        </label>

        <label>Genitore (se minore) <input value={m.genitore_nome || m.genitoreNome || ''} onChange={e => setM({ ...m, genitore_nome: e.target.value })} /></label>
        ...
        <label>Scadenza GAL
          <input
            type="date"
            value={((m.wt_gal_scadenza as string) || '').slice(0, 10)}
            onChange={e => setM({ ...m, wt_gal_scadenza: e.target.value })}
          />
        </label>
      </div>
