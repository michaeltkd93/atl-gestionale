...
export default function Page(){
  ...
  const [form, setForm] = useState<any>({
    nome:'', cognome:'', sede:'Rozzano', telefono:'', email:'',
    genitoreNome:'', genitoreTelefono:'', grado:'Bianca',
    fitaNumero:'', wtGalNumero:'', wtGalScadenza:''
  });
    ...
    <>
      <div className="card">
        <form onSubmit={onSubmit} style={{display:'grid',gridTemplateColumns:'repeat(6,1fr)',gap:10}}>
          ...
          <label>Scadenza GAL
            <input type="date" value={form.wtGalScadenza} onChange={e=>setForm({...form, wtGalScadenza:e.target.value})}/>
          </label>
      ...
      <div className="card">
        <table className="table">
          <thead>
            <tr><th>Atleta</th><th>Sede</th><th>Grado</th><th>FITA</th><th>WT GAL</th><th>Scadenza GAL</th><th></th></tr>
          </thead>
          <tbody>
            {members.map((m:any,i:number)=>(
              <tr key={i}>
                ...
                <td>{m.grado || '-'}</td>
                <td>{m.fita_numero || '-'}</td>
                <td>{m.wt_gal_numero || '-'}</td>
                <td>{m.wt_gal_scadenza || '-'}</td>
                <td><Link href={`/members/${m.id}`}>Scheda</Link></td>
              </tr>
            ))}
          </tbody>
        </table>
