...
export default function Page(){
  ...
  const save=async ()=>{
    const patch = {
      nome: m.nome, cognome: m.cognome, sede: m.sedeNome,
      ...
      grado: m.grado, fitaNumero: m.fita_numero || m.fitaNumero,
  ...
  return (<>
    <div className="header"><h2>Scheda atleta</h2><Link href="/members">← Torna</Link></div>
    <div className="card" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12}}>
      ...
      <label>Sede
        <select value={m.sedeNome || 'Rozzano'} onChange={e=>setM({...m, sedeNome:e.target.value})}>
          <option>Rozzano</option><option>Novara</option>
        </select>
      </label>
